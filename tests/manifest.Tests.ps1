Describe "Manifests" {
    ...
    BeforeAll {
        ...
        $kubeconformignore = Get-Content -Path "./.kubeconformignore" -ErrorAction SilentlyContinue | ForEach-Object { $_.Trim() } | Where-Object { $_ -ne "" } | ForEach-Object { Get-Item -Path $_ }
    ...
    Context "Valid K8s resources" {
        ForEach ($file in Get-ChildItem -Path ./kubernetes/cluster -Include ("*.yaml", "*.yml") -Recurse) {
            It "$($file) should be valid k8s resource" -TestCases @(
                ...
                if ($kubeconformignore -like $file.FullName) {
                    Write-Information "INFO - Skipping $file"
                    return
                }
                $res = kubeconform $kubeconform_config $_.file.FullName
                if ($LASTEXITCODE -ne 0) {
                    Write-Error "ERROR - $res"
                }
                $LASTEXITCODE | Should -BeFalse
            }
        }
